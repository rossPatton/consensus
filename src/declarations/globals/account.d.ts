...
declare global {
  namespace ts {
    ...
    declare type accountBase = tFormSubmit & Readonly<{
      created_at?: string,
    ...
    declare type adminSectionParams = match & {
      params: {
        page?: string,
        section: 'account'
          | 'deleteAccount'
    ...
    // ts.session is like tUser, but with auth data and everything is optional
    // since a user might not be logged in
    declare type session<T = tUser | tGroup> = Partial<tAccountBase>
      & Readonly<{
